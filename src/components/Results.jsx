...
import AnchorOutput from './AnchorOutput';
import FavDropdown from './FavDropdown';
import Watermark from './Watermark';

function Results({ form, calc }) {
          const favBeamData = [
            { label1: calc?.L173, value1: calc?.M173 },
            { label1: calc?.L174, value1: calc?.M174 },
            { label1: calc?.L175, value1: calc?.M175 },
            { label1: calc?.L176, value1: calc?.M176 },
            { label1: calc?.L177, value1: calc?.M177 },
            { label1: calc?.L178, value1: calc?.M178 },
            { label1: calc?.L179, value1: calc?.M179 },
            { label1: calc?.L180, value1: calc?.M180 },
            { label1: calc?.L181, value1: calc?.M181 },
            { label1: calc?.L182, value1: calc?.M182 },
          ];

          const favStrutData = [
            { label1: calc?.L188, value1: calc?.M188 },
            { label1: calc?.L189, value1: calc?.M189 },
            { label1: calc?.L190, value1: calc?.M190 },
            { label1: calc?.L191, value1: calc?.M191 },
            { label1: calc?.L192, value1: calc?.M192 },
            { label1: calc?.L193, value1: calc?.M193 },
            { label1: calc?.L194, value1: calc?.M194 },
            { label1: calc?.L195, value1: calc?.M195 },
            { label1: calc?.L196, value1: calc?.M196 },
            { label1: calc?.L197, value1: calc?.M197 },
          ];

          const favRoofElementData = [
            { label1: calc?.L115, value1: calc?.M115 },
            { label1: calc?.L116, value1: calc?.M116 },
            { label1: calc?.L117, value1: calc?.M117 },
            { label1: calc?.L118, value1: calc?.M118 },
            { label1: calc?.L119, value1: calc?.M119 },
            { label1: calc?.L120, value1: calc?.M120 },
            { label1: calc?.L121, value1: calc?.M121 },
            { label1: calc?.L122, value1: calc?.M122 },
            { label1: calc?.L123, value1: calc?.M123 },
            { label1: calc?.L124, value1: calc?.M124 },
          ];

          // helpers (can live near Outputfield or in a utils file)
          const parsePercent = (v) => {
            if (v == null) return null;
            const m = String(v).match(/-?\d+(\.\d+)?/); // grabs 123 or 123.45 from "123%" etc.
            if (!m) return null;
            const num = parseFloat(m[0]);
            return Number.isFinite(num) ? num : null;
          };

          const pickMaxPercent = (a, b, { precision = 0 } = {}) => {
            const nums = [parsePercent(a), parsePercent(b)].filter((n) => n != null);
            if (nums.length === 0) return null;                // nothing valid
            const max = Math.max(...nums);
            const val = precision == null ? max : +max.toFixed(precision);
            return `${val}%`;                                  // always "x%"
          };

          // utils (put near the component or in a shared utils file)
          const parseLb = (v) => {
            if (v == null) return null;
            const m = String(v).match(/-?\d+(\.\d+)?/); // gets 487 from "487 lb"
            if (!m) return null;
            const n = parseFloat(m[0]);
            return Number.isFinite(n) ? n : null;
          };

          const pickMaxLb = (a, b, { precision = 0 } = {}) => {
            const nums = [parseLb(a), parseLb(b)].filter((n) => n != null);
            if (nums.length === 0) return null; // no valid numbers
            const max = Math.max(...nums);
            const val = precision == null ? max : +max.toFixed(precision);
            // format with thousands separators if you like:
            const pretty = new Intl.NumberFormat().format(val);
            return `${pretty} lb`;
          };
        ...
        <h2 className="text-xl font-semibold mb-2 text-center tracking-wide pt-2">Results</h2>
          ...
          <div className="mt-4 relative isolate w-full max-w-4xl mx-auto hidden sm:block overflow-visible mb-4">
            ...
            <DotLine
              ...
              angle={50}  // aim toward the card (adjust)
              length="10%" // % of container width (adjust)
            />
            ...
            <DotLine
              className="z-0"
              x="25%"      // adjust to the strut feature on the diagram
              y="70%"
              angle={290}
              length="15%"
            />
            ...
            <div
              ...
              style={{
                ...
                transform: "rotate(-32deg)", // control rotation
            ...
            >
              <input
                type="text"
                readOnly
                className="w-full h-full rounded bg-gray-200 text-center"
                value={`${form.canopy_total_x_width_projection ?? ""} ft`}
              />
            </div>
            ...
            >
              <input
                ...
                value={`${form.spacing_between_hanger_arms_or_struts ?? ""} ft`}
              />
            </div>
            ...
            >
              <input
                ...
                readOnly
                className="w-full h-full p-2 border border-gray-300 rounded bg-gray-200"
                value={`${form.canopy_total_y_length ?? ""} ft`}
              />
            </div>
            ...
            <div
              ...
              style={{ top: "1%", left: "2%", width: 220, overflow: "visible" }}
            >
              <Outputfield
                className="relative z-10"
                componentHeading="Hanger Arm Results"
                componentName={form.hanger_arm_item_name}
                designText={pickMaxPercent(calc?.K246, calc?.K251)}
            ...
            <div
              className="absolute pointer-events-auto overflow-visible z-[10] focus-within:z-[2000]"
              style={{ top: "20%", left: "2%", width: 220, overflow: "visible" }}
            >
              <Outputfield
                className="relative z-10"
                componentHeading="Perimeter Beam Results"
                componentName={form.beam_item_name}
                designText={calc?.K179}
                deflectionText={calc?.K180}
              />
            </div>
            ...
            <div
              className="absolute pointer-events-auto overflow-visible z-[10] focus-within:z-[2000]"
              style={{ top: "70%", left: "2%", width: 220, overflow: "visible" }}
            >
              <Outputfield
                className="relative z-10"
                componentHeading="Strut Beam Results"
                componentName={form.strut_beam_item_name}
                designText={calc?.K198}
                deflectionText={calc?.K199}
              />
            </div>
          ...
          {/**Fav components */}
          <div className="bg-white w-full mt-4 rounded-md border">
            <div className="mx-auto max-w-5xl p-4">
              <div className="grid grid-cols-1 gap-3">
                <p className='font-semibold text-center text-md'>Explore the estimated design values for the commonly
                  used components.
                </p>
                <p className='font-normal text-center text-sm italic'>change the selection on the input sections to check.</p>
                <FavDropdown title="Roof Element" data={favRoofElementData} />
                <FavDropdown title="Strut Beam" data={favStrutData} />
                <FavDropdown title="Perimeter Beam" data={favBeamData} />
              </div>
            </div>
          </div>
          ...
          {/**Image2 + overlay (desktop+tablet) --- */}
           <div className="mt-4 relative isolate w-full max-w-4xl mx-auto hidden sm:block overflow-visible mb-4">
            ...
            <Watermark
                  text="Not Valid for construction"
                  angle={-35}
                  size={30}
                  left={0}
                  bottom={300}
              ...
              <div
                className="absolute pointer-events-auto overflow-visible z-[10] focus-within:z-[2000]"
                style={{ top: "1%", left: "5%", width: 220, overflow: "visible" }}
              >
                <div
                  ...
                  <table className='mt-2 w-full border-t border-black'> 
                    <tbody>
                      <tr className="">
                        ...
                        <td className="text-center bg-gray-200 p-1">{pickMaxLb(calc?.K291, calc?.M291) ?? "--"}</td>
                      </tr>
                      <tr className="">
                        <th scope="row" className="text-center font-semibold text-black p-1">Design Tension</th>
                        <td className="text-center bg-gray-200 p-1">{pickMaxLb(calc?.K292, calc?.M292) ?? "--"}</td>
                      </tr>
                      <tr className="">
                        <th scope="row" className="text-center font-semibold text-black p-1">Design Compression</th>
                        <td className="text-center bg-gray-200  p-1">{pickMaxLb(calc?.K293, calc?.M293) ?? "--"}</td>
                      </tr>
                      <tr>
                        <th scope="row" className="text-center font-semibold text-black p-1">Design Moment</th>
                        <td className="text-center bg-gray-200 p-1">{pickMaxLb(calc?.K294, calc?.M294) ?? "--"}</td>
                      </tr>
                    </tbody>
                  </table>
                </div>
              ...
              <div
                className="absolute pointer-events-auto overflow-visible z-[10] focus-within:z-[2000]"
                style={{ top: "1%", left: "35%", width: 220, overflow: "visible" }}
              >
                <div
                  className={`border border-black bg-white p-1 text-sm leading-tight flex flex-col items-center`}
                >
                  ...
                  <table className='mt-2 w-full border-t border-black'> 
                    <tbody>
                      <tr className="">
                        <th scope="row" className="text-center font-semibold text-black p-1">Design Shear</th>
                        <td className="text-center bg-gray-200 p-1">{pickMaxLb(calc?.K298, calc?.M298) ?? "--"}</td>
                      </tr>
                      <tr className="">
                        <th scope="row" className="text-center font-semibold text-black p-1">Design Tension</th>
                        <td className="text-center bg-gray-200 p-1">{pickMaxLb(calc?.K299, calc?.M299) ?? "--"}</td>
                      </tr>
                      <tr className="">
                        <th scope="row" className="text-center font-semibold text-black p-1">Design Compression</th>
                        <td className="text-center bg-gray-200  p-1">{pickMaxLb(calc?.K300, calc?.M300) ?? "--"}</td>
                      </tr>
                      <tr>
                        <th scope="row" className="text-center font-semibold text-black p-1">Design Moment</th>
                        <td className="text-center bg-gray-200 p-1">{pickMaxLb(calc?.K301, calc?.M301) ?? "--"}</td>
                      </tr>
                    </tbody>
                  </table>
                </div>
              ...
              <div
                className="absolute pointer-events-auto overflow-visible z-[10] focus-within:z-[2000]"
                style={{ top: "30%", left: "5%", width: 220, overflow: "visible" }}
              >
              <AnchorOutput
                plateName={form.client_top_wall_plate_options}
                anchorCount={calc?.D318}
                anchorName={calc?.K314}
                hostMaterial={form.host_material}
                designText={pickMaxPercent(calc?.K322, calc?.M322) ?? "--"}
              />
              </div>
              {/** End :Anchor Output1 */}
              ...
              <div
                className="absolute pointer-events-auto overflow-visible z-[10] focus-within:z-[2000]"
                style={{ top: "60%", left: "5%", width: 220, overflow: "visible" }}
              >
              <AnchorOutput
                plateName={form.client_top_wall_plate_options}
                anchorCount={calc?.D318}
                anchorName={calc?.K372}
                hostMaterial= {form.host_material}
                designText={pickMaxPercent(calc?.K380, calc?.M380) ?? "--"}
              />
              </div>
              {/** End :Anchor Output2 */}
            </div>
          {/* --- End Image2 + overlay (desktop/tablet) --- */}

          {/* --- Image2 + overlay (mobile) --- */}
          <div className="sm:hidden space-y-4 mt-4">

            {/* (Optional) small diagram preview; remove if you don't want it on mobile */}
            <div className="rounded-md overflow-hidden border border-black bg-white">
              <img
                src={
                                // Normalize: remove spaces, then test for parallel or perpendicular marks
                                /(\|\||∥)/.test(String(form.roof_element_direction ?? "").replace(/\s/g, ""))
                                  ? diagram4      // parallel: "||" (with or without spaces) or "∥"
                                  : diagram4_4    // perpendicular: anything else (incl. "⊥" / "Ʇ")
                              }
                alt="Canopy layout"
            ...
            <div className="rounded-md shadow-sm border border-black overflow-hidden bg-white">
              ...
              <table className="w-full">
                <tbody>
                  <tr className="border-b border-black">
                    <th scope="row" className="px-4 py-3 text-center font-semibold text-gray-900">Design Shear</th>
                    <td className="px-4 py-3 text-center bg-gray-200 border-l border-black">{pickMaxLb(calc?.K291, calc?.M291) ?? "--"}</td>
                  </tr>
                  <tr className="border-b border-black">
                    <th scope="row" className="px-4 py-3 text-center font-semibold text-gray-900">Design Tension</th>
                    <td className="px-4 py-3 text-center bg-gray-200 border-l border-black">{pickMaxLb(calc?.K292, calc?.M292) ?? "--"}</td>
                  </tr>
                  <tr className="border-b border-black">
                    <th scope="row" className="px-4 py-3 text-center font-semibold text-gray-900">Design Compression</th>
                    <td className="px-4 py-3 text-center bg-gray-200 border-l border-black">{pickMaxLb(calc?.K293, calc?.M293) ?? "--"}</td>
                  </tr>
                  <tr>
                    <th scope="row" className="px-4 py-3 text-center font-semibold text-gray-900">Design Moment</th>
                    <td className="px-4 py-3 text-center bg-gray-200 border-l border-black">{pickMaxLb(calc?.K294, calc?.M294) ?? "--"}</td>
                  </tr>
                </tbody>
              </table>
            </div>
            ...
            <div className="rounded-md shadow-sm border border-black overflow-hidden bg-white">
              ...
              <div className="p-2">
                ...
                anchorName={calc?.K314}
                hostMaterial={form.host_material}
                designText={pickMaxPercent(calc?.K322, calc?.K322) ?? "--"}
              />
              </div>
            </div>
            ...
            <div className="rounded-md shadow-sm border border-black overflow-hidden bg-white">
              ...
              <table className="w-full">
                <tbody>
                  <tr className="border-b border-black">
                    <th scope="row" className="px-4 py-3 text-center font-semibold text-gray-900">Design Shear</th>
                    <td className="px-4 py-3 text-center bg-gray-200 border-l border-black">{pickMaxLb(calc?.K298, calc?.M298) ?? "--"}</td>
                  </tr>
                  <tr className="border-b border-black">
                    <th scope="row" className="px-4 py-3 text-center font-semibold text-gray-900">Design Tension</th>
                    <td className="px-4 py-3 text-center bg-gray-200 border-l border-black">{pickMaxLb(calc?.K299, calc?.M299) ?? "--"}</td>
                  </tr>
                  <tr className="border-b border-black">
                    <th scope="row" className="px-4 py-3 text-center font-semibold text-gray-900">Design Compression</th>
                    <td className="px-4 py-3 text-center bg-gray-200 border-l border-black">{pickMaxLb(calc?.K300, calc?.M300) ?? "--"}</td>
                  </tr>
                  <tr>
                    <th scope="row" className="px-4 py-3 text-center font-semibold text-gray-900">Design Moment</th>
                    <td className="px-4 py-3 text-center bg-gray-200 border-l border-black">{pickMaxLb(calc?.K301, calc?.M301) ?? "--"}</td>
                  </tr>
                </tbody>
              </table>
            </div>
